...
def get_threads_latest_post() -> dict | None:
    ...
    昼(11〜19時)のツリー投稿は絶対に選ばない。選んだ投稿がツリー(連投)なら全部結合して全文にする。"""
    ...
    try:
        ...
        # 連投(ツリー)なら、同じスレッドの全パートを時系列で結合して全文にする
        parts = []
        for it in items:
            rp = it.get("root_post") or {}
            if it.get("id") == root_id or rp.get("id") == root_id:
                t = (it.get("text") or "").strip()
                if not t:
                    continue
                try:
                    pdt = datetime.fromisoformat(it.get("timestamp", "").replace("Z", "+00:00"))
                except Exception:
                    pdt = dt.astimezone(timezone.utc)
                parts.append((pdt, t))
        parts.sort(key=lambda x: x[0])
        full_text = "\n\n".join(t for _, t in parts) if parts else (root.get("text") or "").strip()
        if len(parts) > 1:
            print(f"朝の投稿はツリー（{len(parts)}部）→ 全部結合して投稿")
