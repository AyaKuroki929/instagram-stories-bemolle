...
def generate_content(today: datetime) -> dict:
    ...
    if is_bad_footing:
        ...
        closing_theme = random.choices(
            [
                "感謝・ご縁・気遣い・サロンの雰囲気など自由な切り口",
                "★回数を重ねるごとに皆様の表情が自信に満ちて明るくなっていくのを見るのが嬉しい、"
                "という変化への喜び（一般的な実感として書く。特定の個人の話・数字・誇張にはしない）",
                "皆様の笑顔や穏やかな表情が私たちの喜び・励みになっていること",
            ],
            weights=[50, 25, 25],
        )[0]
        hook_rule = ("② ご来店を心待ちにしている一言。天気や季節の話には触れず、"
                     f"今日の切り口＝{closing_theme}（毎回表現を変える）")
